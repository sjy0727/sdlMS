...
void FreeType::size(int size) { FT_Set_Pixel_Sizes(*face, 0, size); }
...
Sprite* FreeType::str(const std::u16string& s, SDL_Color color, int w)
{
    ...
    if (w <= 0)
    {
        ...
        for (auto& c : s)
        {
            ...
            // 默认顶部对齐
            SDL_Rect char_rect = {offsetX,
                                  (int)(height - glyph_slot->bitmap.rows) / 2,
                                  (int)glyph_slot->bitmap.width,
                                  (int)glyph_slot->bitmap.rows};
            // 特殊符号居中
            if (c == u'<' || c == u'>')
            {
    ...
    else
    {
        ...
        width += 12;
        SDL_Texture* texture =
            SDL_CreateTexture(renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STATIC, width, height * line);
        for (int i = 0; i < s.length(); i++)
        {
            // 获取第l行数
            auto l = i / w + 1;
            ...
            // 默认顶部对齐
            SDL_Rect char_rect = {offsetX,
                                  (int)(height - glyph_slot->bitmap.rows) / 2 + offsetY,
                                  (int)glyph_slot->bitmap.width,
                                  (int)glyph_slot->bitmap.rows};
            // 特殊符号居中
            if (c == u'<' || c == u'>')
