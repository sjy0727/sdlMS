    ...
    if (node != nullptr)
    {
        for (auto& [page, val] : node->get_children())
        {
            for (auto& [zmass, val] : val[0]->get_children())
            {
                ...
                {
                    auto      id = std::stoi(std::string {key.begin(), key.end()});
                    FootHold* f  = new FootHold(val[0],
                                               std::stoi(std::string {page.begin(), page.end()}),
                                               std::stoi(std::string {zmass.begin(), zmass.end()}),
                                               world);
                    f->set_id(id);
...
uint32_t Map::get_map_id() { return map_id; }

void Map::set_map_id(uint32_t id) { map_id = id; }