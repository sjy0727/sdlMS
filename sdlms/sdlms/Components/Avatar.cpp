        ...
        for (auto stance_node : body_node->get_children())
        {
            ...
            for (uint8_t frame = 0; auto frame_node = stance_node.second[0]->get_child(std::to_string(frame)); ++frame)
            {
                ...
                else
                {
                    ...
                    auto        h         = head_node->find_from_path(
                        ststr + u"/" + std::u16string {frame_str.begin(), frame_str.end()} + u"/" + u"head");
                    if (h != nullptr)
                    {
                    ...
                    arm_positions[type][frame] =
                        body_shift_map.count(Layer::ARM) ?
                            (body_shift_map[Layer::ARM][u"hand"] - body_shift_map[Layer::ARM][u"navel"] +
                             body_shift_map[Layer::BODY][u"navel"]) :
                            (body_shift_map[Layer::ARM_OVER_HAIR][u"hand"] -
                             body_shift_map[Layer::ARM_OVER_HAIR][u"navel"] + body_shift_map[Layer::BODY][u"navel"]);

                    hand_positions[type][frame] = body_shift_map[Layer::HAND_BELOW_WEAPON][u"handMove"];

                    head_positions[type][frame] =
                        body_shift_map[Layer::BODY][u"neck"] - body_shift_map[Layer::HEAD][u"neck"];

                    face_positions[type][frame] = body_shift_map[Layer::BODY][u"neck"] -
                                                  body_shift_map[Layer::HEAD][u"neck"] +
                                                  body_shift_map[Layer::HEAD][u"brow"];

                    hair_positions[type][frame] = body_shift_map[Layer::HEAD][u"brow"] -
                                                  body_shift_map[Layer::HEAD][u"neck"] +
                                                  body_shift_map[Layer::BODY][u"neck"];
                }
            }
        }
    }
    ...
    for (uint8_t i = 0; i < ACTION::LENGTH; i++)
    {
        for (uint8_t no = 0; no < body_positions[i].size(); no++)
        {
            ...
            }
            {
                ...
                {
                    auto arm_pos =
                        dynamic_cast<wz::Property<wz::WzVec2D>*>(body_node->find_from_path(type + u"/arm/map/hand"))
                            ->get();
                    Sprite*    sprite = new Sprite(body_node->find_from_path(type + u"/arm"));
                    Transform* f =
                        new Transform(arm_positions[i][no] - SDL_FPoint {(float)arm_pos.x, (float)arm_pos.y});
                    auto z      = std::any_cast<std::u16string>(sprite->get_z());
                    auto part   = *zmap[z];
                    part[i][no] = {f, sprite};
                }
            }
            {
                if (body_node->find_from_path(type + u"/hand") != nullptr)
                {
                    ...
                    {
                        auto hand_pos =
                            dynamic_cast<wz::Property<wz::WzVec2D>*>(hand->find_from_path(u"map/navel"))->get();
                        Sprite*    sprite = new Sprite(hand);
                        Transform* f =
                            new Transform(body_positions[i][no] - SDL_FPoint {(float)hand_pos.x, (float)hand_pos.y});
                        auto z      = std::any_cast<std::u16string>(sprite->get_z());
                        auto part   = *zmap[z];
                        part[i][no] = {f, sprite};
                    }
            ...
            }
            {
                if (body_node->find_from_path(type + u"/lHand") != nullptr)
                {
                    ...
                    {
                        auto lHand_pos =
                            dynamic_cast<wz::Property<wz::WzVec2D>*>(lHand->find_from_path(u"map/handMove"))->get();
                        Sprite*    sprite = new Sprite(lHand);
                        Transform* f =
                            new Transform(hand_positions[i][no] - SDL_FPoint {(float)lHand_pos.x, (float)lHand_pos.y});
                        auto z      = std::any_cast<std::u16string>(sprite->get_z());
                        auto part   = *zmap[z];
                        part[i][no] = {f, sprite};
                    }
                    ...
                    {
                        auto lHand_pos =
                            dynamic_cast<wz::Property<wz::WzVec2D>*>(lHand->find_from_path(u"map/navel"))->get();
                        Sprite*    sprite = new Sprite(lHand);
                        Transform* f =
                            new Transform(body_positions[i][no] - SDL_FPoint {(float)lHand_pos.x, (float)lHand_pos.y});
                        auto z      = std::any_cast<std::u16string>(sprite->get_z());
                        auto part   = *zmap[z];
                        part[i][no] = {f, sprite};
                    }
            ...
            }
            {
                if (body_node->find_from_path(type + u"/rHand") != nullptr)
                {
                    ...
                    {
                        auto rHand_pos =
                            dynamic_cast<wz::Property<wz::WzVec2D>*>(rHand->find_from_path(u"map/navel"))->get();
                        Sprite*    sprite = new Sprite(rHand);
                        Transform* f =
                            new Transform(body_positions[i][no] - SDL_FPoint {(float)rHand_pos.x, (float)rHand_pos.y});
                        auto z      = std::any_cast<std::u16string>(sprite->get_z());
                        auto part   = *zmap[z];
                        part[i][no] = {f, sprite};
                    }
        ...
        for (uint8_t i = 0; i < ACTION::LENGTH; i++)
        {
            for (uint8_t no = 0; no < body_positions[i].size(); no++)
            {
                ...
                auto type   = type_map2.at(i) + u"/" + std::u16string {no_str.begin(), no_str.end()};
                {
                    ...
                    {
                        auto coat_pos = dynamic_cast<wz::Property<wz::WzVec2D>*>(
                                            coat_node->find_from_path(type + u"/mail/map/navel"))
                                            ->get();
                        Sprite*    sprite = new Sprite(coat_node->find_from_path(type + u"/mail"));
                        Transform* f =
                            new Transform(body_positions[i][no] - SDL_FPoint {(float)coat_pos.x, (float)coat_pos.y});
                        auto z      = std::any_cast<std::u16string>(sprite->get_z());
                        auto part   = *zmap[z];
                        part[i][no] = {f, sprite};
                    }
                }
                {
                    ...
                    {
                        auto mail_arm_pos = dynamic_cast<wz::Property<wz::WzVec2D>*>(
                                                coat_node->find_from_path(type + u"/mailArm/map/navel"))
                                                ->get();
                        Sprite*    sprite = new Sprite(coat_node->find_from_path(type + u"/mailArm"));
                        Transform* f      = new Transform(body_positions[i][no] -
                                                     SDL_FPoint {(float)mail_arm_pos.x, (float)mail_arm_pos.y});
                        auto       z      = std::any_cast<std::u16string>(sprite->get_z());
                        auto       part   = *zmap[z];
                        part[i][no]       = {f, sprite};
                    }
            ...
            for (uint8_t no = 0; no < body_positions[i].size(); no++)
            {
                ...
                if (cap_node->find_from_path(type) != nullptr)
                {
                    for (auto it : cap_node->find_from_path(type)->get_children())
                    {
                        ...
                        auto cap_pos =
                            dynamic_cast<wz::Property<wz::WzVec2D>*>(cap->find_from_path(u"map/brow"))->get();
                        Sprite*    sprite = new Sprite(cap);
                        Transform* f =
                            new Transform(face_positions[i][no] - SDL_FPoint {(float)cap_pos.x, (float)cap_pos.y});
                        auto z      = std::any_cast<std::u16string>(sprite->get_z());
                        auto part   = *zmap[z];
                        part[i][no] = {f, sprite};
                    }
    ...
    if (pants_node != nullptr)
    {
        for (uint8_t i = 0; i < ACTION::LENGTH; i++)
        {
            for (uint8_t no = 0; no < body_positions[i].size(); no++)
            {
                ...
                {
                    auto pants_pos =
                        dynamic_cast<wz::Property<wz::WzVec2D>*>(pants_node->find_from_path(type + u"/pants/map/navel"))
                            ->get();
                    Sprite*    sprite = new Sprite(pants_node->find_from_path(type + u"/pants"));
                    Transform* f =
                        new Transform(body_positions[i][no] - SDL_FPoint {(float)pants_pos.x, (float)pants_pos.y});
                    auto z      = std::any_cast<std::u16string>(sprite->get_z());
                    auto part   = *zmap[z];
                    part[i][no] = {f, sprite};
                }
        ...
        for (uint8_t no = 0; no < body_positions[i].size(); no++)
        {
            ...
            if (hairs_node != nullptr)
            {
                for (auto it : hairs_node->get_children())
                {
                    ...
                    Transform* f =
                        new Transform(face_positions[i][no] - SDL_FPoint {(float)hair_pos.x, (float)hair_pos.y});
                    auto z      = std::any_cast<std::u16string>(sprite->get_z());
                    auto part   = *zmap[z];
                    part[i][no] = {f, sprite};
                }
    ...
    if (shoes_node != nullptr)
    {
        for (uint8_t i = 0; i < ACTION::LENGTH; i++)
        {
            for (uint8_t no = 0; no < body_positions[i].size(); no++)
            {
                ...
                {
                    auto shoes_pos =
                        dynamic_cast<wz::Property<wz::WzVec2D>*>(shoes_node->find_from_path(type + u"/shoes/map/navel"))
                            ->get();
                    Sprite*    sprite = new Sprite(shoes_node->find_from_path(type + u"/shoes"));
                    Transform* f =
                        new Transform(body_positions[i][no] - SDL_FPoint {(float)shoes_pos.x, (float)shoes_pos.y});
                    auto z      = std::any_cast<std::u16string>(sprite->get_z());
                    auto part   = *zmap[z];
                    part[i][no] = {f, sprite};
                }
        ...
        for (uint8_t i = 0; i < ACTION::LENGTH; i++)
        {
            for (uint8_t no = 0; no < body_positions[i].size(); no++)
            {
                ...
                if (weapon_node->find_from_path(type + u"/weapon") != nullptr)
                {
                    ...
                    {
                        auto weapon_pos = dynamic_cast<wz::Property<wz::WzVec2D>*>(
                                              weapon_node->find_from_path(type + u"/weapon/map/hand"))
                                              ->get();
                        Sprite*    sprite = new Sprite(weapon_node->find_from_path(type + u"/weapon"));
                        Transform* f =
                            new Transform(arm_positions[i][no] - SDL_FPoint {(float)weapon_pos.x, (float)weapon_pos.y});
                        auto z      = std::any_cast<std::u16string>(sprite->get_z());
                        auto part   = *zmap[z];
                        part[i][no] = {f, sprite};
                    }
                    ...
                    {
                        auto weapon_pos = dynamic_cast<wz::Property<wz::WzVec2D>*>(
                                              weapon_node->find_from_path(type + u"/weapon/map/navel"))
                                              ->get();
                        Sprite*    sprite = new Sprite(weapon_node->find_from_path(type + u"/weapon"));
                        Transform* f      = new Transform(body_positions[i][no] -
                                                     SDL_FPoint {(float)weapon_pos.x, (float)weapon_pos.y});
                        auto       z      = std::any_cast<std::u16string>(sprite->get_z());
                        auto       part   = *zmap[z];
                        part[i][no]       = {f, sprite};
                    }
    ...
    if (shield_node != nullptr)
    {
        for (uint8_t i = 0; i < ACTION::LENGTH; i++)
        {
            for (uint8_t no = 0; no < body_positions[i].size(); no++)
            {
                ...
                {
                    auto shield_pos = dynamic_cast<wz::Property<wz::WzVec2D>*>(
                                          shield_node->find_from_path(type + u"/shield/map/navel"))
                                          ->get();
                    Sprite*    sprite = new Sprite(shield_node->find_from_path(type + u"/shield"));
                    Transform* f =
                        new Transform(body_positions[i][no] - SDL_FPoint {(float)shield_pos.x, (float)shield_pos.y});
                    auto z      = std::any_cast<std::u16string>(sprite->get_z());
                    auto part   = *zmap[z];
                    part[i][no] = {f, sprite};
                }
