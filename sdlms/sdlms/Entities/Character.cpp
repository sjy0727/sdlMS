...
Character::Character(World *world, SDL_FPoint p)
{
    ...
    ava->add_head(u"00012000"); // 头部颜色
    ava->add_body(u"00002003"); // 肤色
    ava->add_coat(u"01040036"); // 上衣
    ava->add_pants(u"01060026");// 裤子
    ava->add_face(u"00020000"); // 面部
    ava->add_hairs(u"00030000"); // 头发
    // ava->add_cap(u"01000016");//Origin
    // ava->add_cap(u"01002906");//辛巴
    ava->add_cap(u"01002140");//GM
    ava->add_shoes(u"01070002"); // 鞋子
    ava->add_weapon(u"01372008"); // 武器
    ava->add_shield(u"01092030"); // 盾牌

    this->add_component(ava);
    world->add_component(ava);

    Transform *tr = new Transform(p);
    this->add_component(tr);
    world->add_component(tr, 3000000);

    // 限制人物移动范围
    LimitTransform *ltr = new LimitTransform(tr, std::nullopt, std::nullopt);
    this->add_component(ltr);
    world->add_component(ltr);

    auto str = new String(u"GM测试");
    this->add_entity(str);
    auto spr = str->get_component<Sprite>();

    auto nam = new NameTag(spr->width, spr->height, u"14"); // 称号tag
    this->add_entity(nam);
    auto rtr = new RelativeTransform(tr, SDL_FPoint{(float)(-nam->get_width() / 2), 6.0f});
    nam->add_component(rtr);
    nam->add_component(new Transform());
    world->add_component(rtr, 0);

    rtr = new RelativeTransform(tr, SDL_FPoint{(float)(-spr->get_width() / 2), 6.0f + (nam->get_height() - spr->height) / 2});
